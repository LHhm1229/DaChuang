...
print('=== Improved Sleep Quality Data Injection ===')
print('Sending 35+ seconds of data per state to form complete epochs\n')

states = ['deep', 'light', 'rem', 'wake']
state_descriptions = {
    'deep': 'Deep Sleep (35s)',
    'light': 'Light Sleep (35s)',
    'rem': 'REM Sleep (35s)',
    'wake': 'Awake (35s)'
}
...
try:
    ...
    while True:
        ...
        # Generate and send 35 seconds of data (enough for 1+ epochs)
        signal = generate_signal_chunk(35, 100, state)
        payload = {
            'rawData': signal,
        ...
        try:
            ...
            if resp.status_code == 200:
                print(f"[Batch {batch_count:3d}] Sent 35s of {state_descriptions[state]:20s} - OK")
            else:
                print(f"[Batch {batch_count:3d}] Error: {resp.status_code}")
        except Exception as e:
            print(f"[Batch {batch_count:3d}] Connection Error: {e}")

        # Wait before sending next batch (let algorithm process)
        time.sleep(5)
