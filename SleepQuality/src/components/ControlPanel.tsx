...
export function ControlPanel({ isMonitoring, onToggleMonitoring }: ControlPanelProps) {
  ...
  const [sensitivity, setSensitivity] = useState(() => {
    const saved = localStorage.getItem('sleep-monitor-sensitivity');
    return saved ? [parseInt(saved)] : [75];
  });
  
  const [audioAlerts, setAudioAlerts] = useState(() => {
    const saved = localStorage.getItem('sleep-monitor-audio-alerts');
    return saved ? JSON.parse(saved) : true;
  });
  
  const [autoSave, setAutoSave] = useState(() => {
    const saved = localStorage.getItem('sleep-monitor-auto-save');
    return saved ? JSON.parse(saved) : true;
  });
  ...
  const [configSampleRate, setConfigSampleRate] = useState(() => {
    const saved = localStorage.getItem('sleep-monitor-config-sampleRate');
    return saved ? parseInt(saved) : 50;
  });
  const [configThreshold, setConfigThreshold] = useState(() => {
    const saved = localStorage.getItem('sleep-monitor-config-threshold');
  ...
  // 保存设置到本地存储并应用到数据服务
  useEffect(() => {
    localStorage.setItem('sleep-monitor-sensitivity', sensitivity[0].toString());
  ...
  useEffect(() => {
    localStorage.setItem('sleep-monitor-audio-alerts', JSON.stringify(audioAlerts));
  }, [audioAlerts]);

  useEffect(() => {
    localStorage.setItem('sleep-monitor-auto-save', JSON.stringify(autoSave));
  }, [autoSave]);
          ...
          <Dialog open={configOpen} onOpenChange={setConfigOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="threshold">睡眠质量阈值 (0-100)</Label>
                  <Input
                    id="threshold"
                    type="number"
                    min={0}
                ...
                <Button
                  onClick={async () => {
                    ...
                    if (Number.isNaN(configThreshold) || configThreshold < 0 || configThreshold > 100) {
                      setConfigError('睡眠质量阈值范围应为 0-100');
                      return;
                    }
                    if (!bluetoothService.isDeviceConnected()) {
                    ...
                    try {
                      ...
                      // 持久化本地
                      localStorage.setItem('sleep-monitor-config-sampleRate', String(configSampleRate));
                      localStorage.setItem('sleep-monitor-config-threshold', String(configThreshold));
                      setConfigOpen(false);
                    } catch (e) {
