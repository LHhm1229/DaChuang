...
// 简化的数据接口 - 只包含原始数据
export interface BluetoothSleepData {
  rawData: RawSensorData;
  timestamp: number;
  // TODO: 在这里添加您的算法处理结果
  // 例如: sleepScore?: number;
...
class BluetoothService {
  ...
  private listeners: ((data: BluetoothSleepData) => void)[] = [];
  ...
  // TODO: 在这里添加您的算法相关变量
  // 例如: private sleepAlgorithm: SleepQualityAlgorithm;
  ...
  // TODO: 在这里实现您的数据处理算法
  // 将原始信号转换为睡眠质量监测数据
  private processRawDataToSleepData(rawData: RawSensorData): BluetoothSleepData {
    // 将原始数据添加到缓冲区
...
==================`;
    ...
    // TODO: 在这里调用您的算法
    // 例如: const sleepScore = this.yourSleepAlgorithm(rawData.values);
  ...
  // TODO: 在这里添加您的算法函数
  // 例如: private yourSleepAlgorithm(values: number[]): number { ... }
    ...
    try {
      ...
      this.deviceInfo.characteristic.addEventListener('characteristicvaluechanged', (event) => {
        ...
        if (value) {
          ...
          if (rawData) {
            // 处理原始数据（在终端显示）
            const processedData = this.processRawDataToSleepData(rawData);
  ...
  // 解析从单片机接收的数据
  private parseDataFromBuffer(buffer: DataView): BluetoothSleepData | null {
    try {
      ...
      // 转换为标准格式（JSON数据解析）
      const processedData: BluetoothSleepData = {
        rawData: {
          timestamp: Date.now(),
          values: [rawData.sleepScore || 0, rawData.blinkRate || 0],
  ...
  // 计算警报级别
  private calculateAlertLevel(sleepScore: number): 'normal' | 'warning' | 'danger' {
    if (sleepScore < 30) return 'danger';
    if (sleepScore < 70) return 'warning';
    return 'normal';
  }
  ...
  // 添加数据监听器
  addListener(listener: (data: BluetoothSleepData) => void) {
    this.listeners.push(listener);
  }

  // 移除数据监听器
  removeListener(listener: (data: BluetoothSleepData) => void) {
  ...
  // 通知所有监听器
  private notifyListeners(data: BluetoothSleepData) {
