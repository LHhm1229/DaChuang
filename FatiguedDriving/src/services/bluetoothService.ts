...
// 简化的数据接口 - 只包含原始数据
export interface BluetoothFatigueData {
  rawData: RawSensorData;
  timestamp: number;
  // TODO: 在这里添加您的算法处理结果
  // 例如: fatigueScore?: number;
...
class BluetoothService {
  ...
  private listeners: ((data: BluetoothFatigueData) => void)[] = [];
  ...
  // TODO: 在这里添加您的算法相关变量
  // 例如: private fatigueAlgorithm: FatigueDetectionAlgorithm;
  ...
  // TODO: 在这里实现您的数据处理算法
  // 将原始信号转换为疲劳监测数据
  private processRawDataToFatigueData(rawData: RawSensorData): BluetoothFatigueData {
    // 将原始数据添加到缓冲区
...
==================`;
    ...
    // TODO: 在这里调用您的算法
    // 例如: const fatigueScore = this.yourFatigueAlgorithm(rawData.values);
  ...
  // 发送数据到后端
  private async sendDataToBackend(payload: any) {
    try {
      await fetch('http://localhost:3001/api/bluetooth-data', {
        method: 'POST',
        headers: {
      ...
      console.error('转发蓝牙数据到后端失败:', error);
    }
  }

  // TODO: 在这里添加您的算法函数
  // 例如: private yourFatigueAlgorithm(values: number[]): number { ... }
    ...
    try {
      ...
      this.deviceInfo.characteristic.addEventListener('characteristicvaluechanged', (event) => {
        ...
        if (value) {
          ...
          if (rawData) {
            // 处理原始数据（在终端显示）
            const processedData = this.processRawDataToFatigueData(rawData);
  ...
  // 解析从单片机接收的数据
  private parseDataFromBuffer(buffer: DataView): BluetoothFatigueData | null {
    try {
      ...
      // 转换为标准格式（JSON数据解析）
      const processedData: BluetoothFatigueData = {
        rawData: {
          timestamp: Date.now(),
          values: [rawData.fatigueScore || 0, rawData.blinkRate || 0],
  ...
  // 计算警报级别
  private calculateAlertLevel(fatigueScore: number): 'normal' | 'warning' | 'danger' {
    if (fatigueScore < 30) return 'normal';
    if (fatigueScore < 70) return 'warning';
    return 'danger';
  }
  ...
  // 添加数据监听器
  addListener(callback: (data: BluetoothFatigueData) => void) {
    this.listeners.push(callback);
  }

  // 移除数据监听器
  removeListener(callback: (data: BluetoothFatigueData) => void) {
  ...
  // 通知所有监听器
  private notifyListeners(data: BluetoothFatigueData) {
